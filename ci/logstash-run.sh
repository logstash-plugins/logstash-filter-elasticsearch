...
#  bundle exec rspec -fd spec/filters -t ~integration -t ~secure_integration
#else
#
#  if [[ "$SECURE_INTEGRATION" == "true" ]]; then
...
#  fi
  ...
  wait_for_es
  bundle exec rspec -fd $extra_tag_args --tag es_version:$ELASTIC_STACK_VERSION spec/filters/integration
#fi
